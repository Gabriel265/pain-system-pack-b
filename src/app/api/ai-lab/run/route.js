/*
This the main route that runs everything for the AI agent to get the prompt and repo tree
*/
...
export async function POST(req) {
  try {
    ...
    const { data: tree } = await octokit.request('GET /repos/{owner}/{repo}/git/trees/ai-deploy?recursive=1', {
    ...
    const contents = {};
    for (const path of paths.slice(0, 60)) {  // safety limit
      try {
...
Respond with:
{
  ...
  "files": [
    ...
    if (!response.summary || !Array.isArray(response.files) || response.files.length === 0) {
...
Review carefully before merging.`;
    ...
    return NextResponse.json({
      ...
      previewUrl,  // ← return the correct one immediately
