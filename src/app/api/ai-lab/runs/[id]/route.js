...
export async function GET(request, context) {  // context contains { params }
  try {
    ...
    if (!pull_number || isNaN(pull_number)) {
      return NextResponse.json({ error: 'Invalid pull request ID' }, { status: 400 });
    }
    ...
    const aiResponseMatch = pr.body?.match(/\*\*AI Response:\*\*\n```json\n([\s\S]*?)\n```/);
    ...
    const run = {
      ...
      aiResponse: aiResponseMatch ? aiResponseMatch[1].trim() : 'No AI response available',
    };
