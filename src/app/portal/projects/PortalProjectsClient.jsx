...
export default function PortalProjectsClient() {
    ...
    for (const id of selectedIds) {
      ...
      if (action === "delete") {
        ...
        await fetch("/api/projects", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ...project, status: "Concept" }),
        });
      }
    }
  ...
  const selectAll = () => {
    if (
      selectedIds.size === currentProjects.length &&
          ...
          {selectedIds.size > 0 && (
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium"
              >
                ...
                <thead className="bg-white  sticky top-0 z-20 shadow-sm">
                  <tr>
                    ...
                    >
                      ...
                      <span
                        onMouseDown={(e) =>
                          startResize("description", e.clientX)
                        }
                        className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-black/20"
                      />
                    </th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {loading ? (
                    ...
                    currentProjects.map((project) => (
                      <tr
                        key={project.id}
                        className="hover:bg-white dark:hover:bg-gray-700 transition"
                      >
                        ...
                        <td className="px-4 py-4 font-medium text-black  truncate max-w-64">
                        ...
                        <td className="px-4 py-4">
                          <span
                            className={`inline-flex px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap ${project.status === "Live" ? "bg-green-100 text-green-800 " : project.status === "In-Build" ? "bg-yellow-100 text-yellow-800" : "bg-gray-100 text-gray-700"}`}
                          >
                        ...
                        <td className="px-4 py-4 text-right">
                          <div className="inline-flex items-center gap-1 bg-gray-100  rounded-lg p-1.5">
                            ...
                            <button
                              ...
                              className="p-2 text-orange-600 hover:bg-orange-100 dark:hover:bg-orange-900 rounded transition"
                            >
        ...
        <div className="lg:hidden space-y-4">
          {loading ? (
            ...
            currentProjects.map((project) => (
              <div
                key={project.id}
                className={`bg-white  rounded-xl shadow-md p-5 border ${selectedIds.has(project.id) ? "border-orange-500" : "border-gray-200 "} transition-all`}
              >
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${project.status === "Live" ? "bg-green-100 text-green-800 " : project.status === "In-Build" ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200" : "bg-gray-100 text-gray-700  "}`}
                    >
                      {project.status || "Concept"}
                    </span>
                ...
                <div className="mt-4 flex justify-end gap-2">
                  <button
                    onClick={() => openEditModal(project)}
                    className="p-2.5 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-800 transition"
                  >
                    <Edit className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleToggleStatus(project)}
                    className="p-2.5 bg-orange-100 dark:bg-orange-900 text-orange-600 dark:text-orange-300 rounded-lg hover:bg-orange-200 dark:hover:bg-orange-800 transition"
                  >
                    <Globe className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleDelete(project.id)}
                    className="p-2.5 bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-300 rounded-lg hover:bg-red-200 dark:hover:bg-red-800 transition"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${currentPage === pageNum ? "bg-orange-500 text-white" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
                    >
                      {pageNum}
                    </button>
                  );
          ...
          <div className="bg-white  rounded-2xl shadow-2xl max-w-2xl w-full max-h-screen overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <input
                ...
                required
                className="w-full px-5 py-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-white  focus:ring-2 focus:ring-orange-500 transition"
              />
              <input
                type="text"
                ...
                required
                className="w-full px-5 py-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-white  font-mono text-sm focus:ring-2 focus:ring-orange-500 transition"
              />
              <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <select
                  ...
                  className="px-5 py-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-white "
                >
              ...
              <div className="flex gap-4">
                <button
                  type="submit"
                  className="flex-1 bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white font-bold py-4 rounded-xl text-lg shadow-xl transition transform hover:scale-105"
                >
                ...
                <button
                  type="button"
                  onClick={closeModal}
                  className="px-8 py-4 bg-gray-200  text-gray-800  font-medium rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition"
                >
                  Cancel
                </button>
              </div>
