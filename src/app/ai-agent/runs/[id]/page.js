...
export default function RunDetailPage() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-lg font-medium text-gray-700">Loading proposal...</p>
      </div>
    </div>
  );
  ...
  if (!run) return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <p className="text-lg text-gray-600">Proposal not found or loading failed.</p>
    </div>
  );
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 max-w-[1600px] mx-auto p-4 md:p-6">
        ...
        <div className="flex-1 min-w-0 transition-all duration-300">
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 md:p-6">
            <h2 className="text-xl md:text-2xl lg:text-3xl font-semibold text-gray-900 mb-4 md:mb-6">
              Code Changes
            </h2>
                ...
                run.files.map((file, i) => (
                  <div key={i} className="border border-gray-300 rounded-lg overflow-hidden shadow-sm">
                    ...
                    <div className="overflow-auto" style={{ maxHeight: '600px' }}>
                      <pre className="p-4 md:p-6 bg-gray-50 text-xs md:text-sm font-mono leading-relaxed overflow-x-auto">
                        <code className="block min-w-0">{file.diff}</code>
                      </pre>
                    </div>
                  </div>
                ))
